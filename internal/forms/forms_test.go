...
func TestForm_Required(t *testing.T) {
	...
	if form.Valid() {
		...
		form.Required("a", "b", "c")
		if !form.Valid() {
			t.Error("shows does not have required fields whend it does")
		}
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("shows minlength of 1 didn't met when data is bigger")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("x")
	if form.Valid() {
		t.Error("form shows email valid for non-existent field")
	}
	...
	form.IsEmail("email")
	if !form.Valid() {
		t.Error("form shows not valid email when it should")
	}
